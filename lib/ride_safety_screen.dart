...
class _RideSafetyScreenState extends State<RideSafetyScreen> {
      ...
      if (_selectedContactIds.contains(id)) {
        ...
        if (_selectedContactIds.length >= _maxRideContacts) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('You can select up to $_maxRideContacts contacts only')),
          );
          return;
        }
  ...
  Future<void> _startRide() async {
    if (_driverNameController.text.trim().isEmpty ||
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please fill in driver and vehicle details')),
      );
      return;
    }
  ...
  Future<void> _showRideInProgressNotification() async {
    const androidDetails = AndroidNotificationDetails(
      ...
      autoCancel: false,
      actions: [
        AndroidNotificationAction('end_ride', 'End Ride'),
      ],
    );
    const details = NotificationDetails(android: androidDetails);
  ...
  Future<void> _sendRideDetails() async {
    ...
    try {
      ...
      await _supabase.from('ride_safety_shares').update({
        'user_consented': true,
        'consented_at': DateTime.now().toIso8601String(),
        'shared_with': selectedContacts,
      }).eq('id', _activeRideId!);

      // TODO: actually send the SMS via Twilio Edge Function here.
      // Not built yet - this records consent and the recipient list only.
        ...
        child: Column(
          ...
          children: [
            ...
            if (_contacts.isEmpty)
              const Text('No emergency contacts saved yet. Add some in Contacts first.')
            else
